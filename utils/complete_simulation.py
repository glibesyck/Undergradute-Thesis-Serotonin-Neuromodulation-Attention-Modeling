...
def network_simulation_run(Ne: int, Ni: int, T: np.ndarray, delta_T: float, start_state: np.ndarray, weights: np.ndarray, currents: np.ndarray, spike_trains: np.ndarray, serotonin_weights: np.ndarray, params: dict):
    ...
    for i in range(1, len(T)):
        ...
        # МАЄ бути щось ось таке np.sum(np.multiply(weights, conductances[i-1, :,  0]), axis = 0) - conductances now contain only pre-synaptic 
        ...
        conductances[i, exc_mask, :Ne, 0] = conductances[i - 1, exc_mask, :Ne, 0] + np.multiply(params["g_ee_ampa"][exc_mask, :Ne], weights[exc_mask, :Ne])
        conductances[i, exc_mask, :Ne, 1] = conductances[i - 1, exc_mask, :Ne, 1] + np.multiply(params["g_ee_nmda"][exc_mask, :Ne], weights[exc_mask, :Ne])

        #excitatory -> inhibitory (notice that both channels have different associated constants)
        conductances[i, exc_mask, Ne:, 0] = conductances[i - 1, exc_mask, Ne:, 0] + np.multiply(params["g_ei_ampa"][exc_mask, Ne:], weights[exc_mask, Ne:])
        conductances[i, exc_mask, Ne:, 1] = conductances[i - 1, exc_mask, Ne:, 1] + np.multiply(params["g_ei_nmda"][exc_mask, Ne:], weights[exc_mask, Ne:])

        #for inhibitory neurons only GABA (A & B) should change 

        #inhibitory -> excitatory
        conductances[i, inh_mask, :Ne, 2] = conductances[i - 1, inh_mask, :Ne, 2] + np.multiply(params["g_ie"][inh_mask, :Ne], weights[inh_mask, :Ne])
        conductances[i, inh_mask, :Ne, 3] = conductances[i - 1, inh_mask, :Ne, 3] + np.multiply(params["g_ie"][inh_mask, :Ne], weights[inh_mask, :Ne])

        #inhibitory -> excitatory
        conductances[i, inh_mask, Ne:, 2] = conductances[i - 1, inh_mask, Ne:, 0] + np.multiply(params["g_ii"][inh_mask, Ne:], weights[inh_mask, Ne:])
        conductances[i, inh_mask, Ne:, 3] = conductances[i - 1, inh_mask, Ne:, 1] + np.multiply(params["g_ii"][inh_mask, Ne:], weights[inh_mask, Ne:])
        ...
        background_conductance[i, external_exc_inv_mask] = background_conductance[i - 1, external_exc_inv_mask] - delta_T*np.divide(background_conductance[i - 1, external_exc_inv_mask], params["tau_ampa"][0, external_exc_inv_mask])

        #for inhibitory neurons who doesn't fire
        background_conductance[i, external_inh_inv_mask] = background_conductance[i - 1, external_inh_inv_mask] - delta_T*np.divide(background_conductance[i - 1, external_inh_inv_mask], params["tau_ampa"][0, external_inh_inv_mask])
